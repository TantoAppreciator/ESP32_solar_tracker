...
void loop() {
  ...
  delay(60*10*1000);  //wait 10 minutes
  adjustPanel();
}
...
void adjustPanel() {
  ...
  do{
    Serial.println("Caution! Elevation below zero");
    timeAcquired = false;
  ...
  while (!timeAcquired) {
    ...
    if (!getLocalTime(&timeinfo)) {
      Serial.println("Failed to acquire time data! Retrying after some time...");
      blinkLED(WAIT_LONG,4);
      //reset to defaults somehow
    } else {
      Serial.print("Time data acquired: ");
      Serial.println(&timeinfo, "%A, %B %d %Y %H:%M:%S");
      utc = mktime(&timeinfo);
      Serial.print("Time data Unix timestamp: ");
      Serial.println(utc);
      timeAcquired = true;
    }
  }
  calcHorizontalCoordinates(utc, lat, lon, azi, elev);
  Serial.printf("Coordinates calculated: azimuth: %f, elevation: %f...\n", azi, elev);
  if (elev < 0)
    delay(60*10*1000);
  } while (elev < 0);
